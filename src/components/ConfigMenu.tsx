...
export const ConfigMenu: React.FC<ConfigMenuProps> = ({ onBack, onGraphicsChanged }) => {
  ...
  const [graphicsPreset, setGraphicsPreset] = useState(graphicsSystem.getPresetName());

  useEffect(() => {
    setSettings(inputSystem.getSettings());
    setGraphicsPreset(graphicsSystem.getPresetName());
  }, []);
  ...
  const handleQualityChange = (quality: 'low' | 'medium' | 'high' | 'ultra') => {
    graphicsSystem.setQuality(quality);
    setGraphicsPreset(quality);
    if (onGraphicsChanged) {
      onGraphicsChanged();
    }
  };
    ...
    <div className="menu-overlay">
      ...
      <div className="menu-content" style={{ width: '700px', maxWidth: '95vw' }}>
        ...
        <div className="config-section" style={{ marginBottom: '20px' }}>
          ...
          <div className="profile-selector" style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
            {['low', 'medium', 'high', 'ultra'].map((preset) => (
              <button
                ...
                className="menu-button"
                style={{
                  backgroundColor: graphicsPreset === preset ? '#8b0000' : '',
                  width: 'auto',
                  flex: 1
                }}
          ...
          <div className="profile-description" style={{ marginTop: '15px', color: '#ccc', fontSize: '14px', minHeight: '20px', textAlign: 'center' }}>
            {graphicsPreset === 'low' && "Niedrige Auflösung (64x64), keine Mipmaps. Schnell."}
            {graphicsPreset === 'medium' && "Mittlere Auflösung (128x128), bilineare Filterung. Ausgewogen."}
            {graphicsPreset === 'high' && "Hohe Auflösung (256x256), Lichteffekte. Schick."}
            {graphicsPreset === 'ultra' && "Ultra Auflösung (512x512), maximale Details. Anspruchsvoll."}
          </div>
        </div>
